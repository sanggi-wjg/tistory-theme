...
현재 상위 11종 / 하위 36종. 실측에서 드러난 문제는 넷이다.

**① `IT`가 미분류 통이다.** 122편(45%)이 `IT` 아래 있고, 그 직속 35편은 서로 관계가 없다 — 보안 사고 대응, Jenkins 설치, SOLID 원칙, IntelliJ 팁, 객체지향 책 정리가 한 목록에 섞인다. 목록 페이지가 정보를 주지 못한다.

**② 분류 축이 세 개 섞여 있다.** 같은 레벨에 언어(`Python` `Go` `Kotlin & Java` `PHP`), 기술 영역(`Infrastructure` `Database` `Search Engine`), 주제(`Clean Code` `리팩토링` `알고리즘`)가 공존한다. "JPA 락"은 언어(Kotlin)인가 영역(Database)인가 — 답이 없다.

**③ 최신 글이 갈 곳이 없다.** 2025~2026년 글은 운영 트러블슈팅과 전환 회고(OOMKilled 추적기, AccessKey 유출 대응, Argo Workflows 전환, AI 에이전트 테스트 인프라)인데, 축이 "언어별 문법 정리"에 맞춰져 있어 `IT` 직속으로 밀려났다.

**④ 잔가지가 많다.** 1편짜리 하위 7개(`Docker` `Mac` `수학` `ElasticSearch` `Lucene Solr` `Laravel` `Codeigniter`)와 **0편짜리 `Design Pattern`**. 사이드바 트리가 47줄인데 그중 상당수가 클릭할 이유가 없는 항목이다.

> 크롤링은 글이 있는 하위 30종만 잡는다. DECISIONS.md §3의 하위 36종과 차이가 나는 만큼(`Design Pattern` 포함 6종 안팎)은 **글이 0편인 카테고리**다. 관리 화면에서 직접 확인해 함께 정리한다.

---

## 2. 새 카테고리 트리

**축을 주제 하나로 통일한다.** 언어는 분류 기준에서 내리고(태그로 대체), "무엇을 다루는 글인가"로만 나눈다.

| 상위 | 편수 | 하위 |
|---|---:|---|
| **언어 & 런타임** | 47 | Python 기초 & 도구 (9) · Python 성능 & 동시성 (14) · Python 활용 & 오픈소스 (8) · JVM (Java · Kotlin) (5) · Go (11) |
| **인프라 & 운영** | 40 | 쿠버네티스 & 컨테이너 (8) · 로그 & 모니터링 (15) · 리눅스 서버 운영 (6) · 네트워크 (6) · CI/CD (5) |
| **백엔드 & 아키텍처** | 39 | Spring (4) · 웹 프레임워크 (12) · 분산 시스템 & MSA (12) · 설계 원칙 & 패턴 (11) |
| **데이터** | 39 | MySQL (19) · 데이터베이스 기초 (4) · NoSQL & 검색 엔진 (5) · 데이터 파이프라인 (11) |
| **코드 품질** | 31 | Clean Code (16) · 리팩토링 (9) · 테스트 (6) |
| **웹 & 보안** | 23 | HTTP & 브라우저 (10) · 보안 (13) |
| **AI** | 20 | LLM 활용 & 에이전트 (6) · Langchain (9) · 로컬 모델 (5) |
| **알고리즘 & CS** | 15 | *(하위 없음)* |
| **개발 도구** | 14 | Git (9) · 에디터 & 터미널 (5) |
| **기록** | 7 | 책 (2) · 일상 (2) · 개발자 생각 (3) |

**상위 10종 · 하위 31종 · 275편.** 상위 편수가 47~7로 퍼지고, 최대 카테고리가 전체의 17%다 (현재는 `IT` 45%).

### 배치를 가른 판단들

- **언어는 축에서 내렸지만 Python만 예외로 남겼다.** Python 31편은 다른 언어와 달리 문법·성능·활용으로 나눌 만큼 두껍고, 검색 유입도 `[Python] ...` 제목에 붙어 있다. Go 11편·JVM 5편은 쪼갤 부피가 아니라 그대로 뒀다.
- **K8s 위에서 Spring 앱을 운영한 6편은 인프라로 보냈다.** OOMKilled 3부작과 Argo 전환 3부작은 Spring 코드보다 K8s 운영이 주제이고, 유입 검색어도 "OOMKilled" "Argo Workflows"일 가능성이 높다. 결과적으로 `Spring`은 4편으로 얇아진다.
- **`Web` 18편은 성격이 둘이었다.** HTTP·브라우저 개념 10편과 웹해킹·XSS 8편이 한 카테고리에 있었다. 후자를 보안으로 모으니 최신 보안 글(타이밍 어택, AccessKey 유출, 개인정보보호법)과 합쳐져 13편이 됐다.
- **`Search Engine` 2편은 단독으로 두지 않았다.** Redis와 묶어 `NoSQL & 검색 엔진` 5편으로. 비-RDB 저장소라는 공통점이 있다.
- **토끼책(객체지향의 사실과 오해) 2편은 `기록/책`이 아니라 `설계 원칙 & 패턴`에 뒀다.** SOLID·DRY와 같은 내용이라 함께 있어야 유용하다. `기록/책`은 후기성 글(쿠버네티스 패턴, 프로그래머의 뇌)만 남긴다.
- **`알고리즘 & CS`만 하위를 두지 않았다.** 문제 풀이 14편 외에 개념 글이 1편(정규화와 표준화)뿐이라 하위를 나눌 실익이 없다.
...
| 현재 | 편수 | 이동 |
|---|---:|---|
| `IT` (직속) | 35 | **11곳으로 분해** — 설계 원칙 11 · 보안 5 · 테스트 4 · 에디터 & 터미널 4 · CI/CD 4 · 개발자 생각 2 · 나머지 5 |
| `Python/Python` | 28 | 성능 & 동시성 14 · 기초 & 도구 9 · 활용 & 오픈소스 4 · 알고리즘 1 |
| `IT/Web` | 18 | HTTP & 브라우저 9 · 보안 8 · 개발자 생각 1 |
| `Database/MySQL` | 21 | MySQL 17 · 데이터베이스 기초 4 |
| `Kotlin & Java/Spring` | 10 | **쿠버네티스 & 컨테이너 6** · Spring 4 |
| `OS/Centos` | 10 | 리눅스 서버 운영 6 · 로그 & 모니터링 2 · 네트워크 2 |
| `IT/AI` | 12 | LLM 활용 & 에이전트 6 · 로컬 모델 5 · Langchain 1 |

**통째로 옮겨가는 것** — `IT/Clean Code`→코드 품질/Clean Code, `IT/리팩토링`→코드 품질/리팩토링, `Infrastructure/MSA`→분산 시스템 & MSA, `Infrastructure/Fluentd`→로그 & 모니터링, `Go`→언어 & 런타임/Go, `Python/Django`+`Python/Flask`+`PHP/*`→웹 프레임워크, `Infrastructure/Kafka`+`CDC`+`Airflow`→데이터 파이프라인.

전체 275편의 개별 이동은 §7 부록과 `data/category-mapping.json`에 있다.
...
| 글 | 배치 | 다른 선택지 |
|---|---|---|
| 캐시 (Cache) | 웹 & 보안/HTTP & 브라우저 | HTTP 캐시가 아니라 애플리케이션 캐시면 → 데이터/NoSQL & 검색 엔진 |
| OOMKilled 추적기 3부작 | 인프라/쿠버네티스 & 컨테이너 | Spring Boot 앱 문제로 보면 → 백엔드/Spring |
| Argo Workflows 전환 3부작 | 인프라/쿠버네티스 & 컨테이너 | 2편이 Spring Batch REST API 설계라 → 백엔드/Spring |
| ChatGPT + streamlit 웹앱 | 언어/Python 활용 & 오픈소스 | → AI/LLM 활용 & 에이전트 |
| 속도의 병목이 이동하고 있다 | AI/LLM 활용 & 에이전트 | 에세이라 → 기록/개발자 생각 |
| 토끼책 2편 | 백엔드/설계 원칙 & 패턴 | 책 정리라 → 기록/책 |
| batch 프로그램으로 host 변경 | 개발 도구/에디터 & 터미널 | hosts 파일이 주제라 → 인프라/네트워크 |
| fail2ban | 인프라/리눅스 서버 운영 | 침입 차단이라 → 웹 & 보안/보안 |

**추가로 크롤링이 닿지 않은 것** — 티스토리 관리 화면의 `비공개용 (3)`은 비공개 글이라 실측에 잡히지 않았다. 이 3편은 직접 확인해서 배치하거나 비공개인 채로 남긴다.
...
| 항목 | 영향 |
|---|---|
| **DECISIONS.md 결정 5** (상위 카테고리별 기본 이미지 11장) | **11장 → 10장.** 상위가 11종에서 10종으로 줄어든다 |
...
| **DESIGN.md §6.2** (`data-cat` 접두 선택자) | **선택자 값 전면 교체.** 새 상위 10종 이름 사이에 접두 충돌은 없다. 다만 이름에 공백과 `&`가 들어가므로 속성 선택자를 반드시 따옴표로 감싼다 — `[data-cat^="언어 & 런타임"]` |
| **DECISIONS.md §3 실측 카테고리 절** | 개편 적용 후 `/blog-census`로 `data/categories.json` 재생성 필요 |
| **DECISIONS.md 미결 6** (기본 이미지 도안) | 이 개편이 그 선행 작업이다. 개편 확정 후 10장 도안 착수 |
...
2. **새 상위 10종을 만든다** — 기존 것을 고치지 말고 새로 만든다. 기존 트리는 이동이 끝날 때까지 남겨 둔다
3. **하위 31종을 만든다**
4. **글을 옮긴다** — 관리 → 글 관리에서 카테고리로 필터 → 전체 선택 → 카테고리 이동. §7 부록의 카테고리별 목록 순서대로 진행하면 한 번에 한 덩어리씩 끝난다
5. **빈 기존 카테고리를 지운다** — 글을 다 옮기면 기존 트리 47줄이 전부 0편이 된다. `Design Pattern`처럼 처음부터 0편이던 것들도 이때 함께 사라진다
6. **비공개 3편을 배치한다**
7. **재실측** — `/blog-census` 실행 → `data/categories.json`·`DECISIONS.md` §3 갱신
8. **스킨 반영** — 기본 이미지 10장 도안 → `DESIGN.md` §6.2 `data-cat` 선택자 교체
...
사이드바 트리와 카테고리 URL에서는 짧은 이름이 낫다. 정보량과 맞바꾸는 선택이라 취향에 맞게 고른다.
...
`&`는 현재 블로그의 `Kotlin & Java`에서 이미 쓰이고 있어 티스토리가 처리한다는 것은 확인됐다. 다만 카테고리 URL에 `%26`으로 인코딩되므로, 짧은 이름을 쓰면 주소도 깨끗해진다.

---

## 8. 부록 — 신규 카테고리별 전체 글 목록

이동 작업 시 체크리스트로 쓴다.

### 언어 & 런타임/Python 기초 & 도구 (9)
