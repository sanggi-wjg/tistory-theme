...
**상위는 언어 이름만 쓴다.** `Kotlin·Spring`이 아니라 `Kotlin·Java`인 이유다 — `Python`이 `Python·Django`가 아닌 것과 같다. **프레임워크는 언어 아래로 간다**(`Python/Django·Flask`, `Kotlin·Java` 안의 Spring 4편). 프레임워크를 언어에서 떼어 별도 상위에 두면 "Spring은 웹 프레임워크가 아닌가?" 하는 질문에 답할 수 없다.

### 3.2 배치를 가른 판단들

- **`설계 원칙` 11편은 `아키텍처` 아래로.** SOLID·객체지향·API 설계는 "품질"이 아니라 "설계"다. `아키텍처`가 `분산 시스템`(시스템 설계 12) + `원칙`(코드 설계 11)이 되면서 설계가 한곳에 모였고, `코드 품질`은 Clean Code·리팩토링·테스트 31편으로 이름과 내용이 맞는다.
- **`데이터 파이프라인` 11편은 `데이터베이스`가 아니라 `인프라`로.** Kafka·Airflow·Debezium은 저장소가 아니라 운영하는 미들웨어다.
- **`웹·보안`을 `네트워크`와 `보안`으로 쪼갰다.** 원래 "DNS 작동원리"는 HTTP 쪽에, "통신사 DNS IP 리스트"는 네트워크 쪽에 흩어져 있었다. **`보안`은 최근 2년 4편에 최신 글이 2026.08로 성장 중이다.**
- **K8s 위에서 Spring 앱을 운영한 6편은 `인프라/쿠버네티스`로.** OOMKilled 3부작과 Argo 전환 3부작은 Spring 코드보다 K8s 운영이 주제이고, 유입 검색어도 "OOMKilled" "Argo Workflows"일 가능성이 높다. 그 대가로 `Kotlin·Java`가 9편으로 얇다.
- **PHP 2편은 `인프라/리눅스`로.** 제목이 `Centos7 Nginx, PHP, MySQL Codeigniter 프로젝트 세팅`과 `Centos Laravel 설치`다. 언어 글이 아니라 서버 설치 글이다.
- **토끼책(객체지향의 사실과 오해) 2편은 `기록`이 아니라 `아키텍처/원칙`에.** SOLID·DRY와 같은 내용이라 함께 있어야 유용하다.
...
**이건 편집 판단이지 기계적 규칙이 아니다.** 그렇게 부르지 않으면 규칙 이름과 실제 순서가 어긋난 채로 굳는다 — 실제로 한 번 그랬다(§4 각주). 새 카테고리가 생기면 **먼저 몇 군인지 정하고**, 1군이면 손으로 자리를 잡는다.
...
```
인프라 47(7) · AI 20(4) · 보안 13(4) · Kotlin·Java 9(4) · 개발 도구 14(3)
· 아키텍처 23(3) · 코드 품질 31(3) · 네트워크 16(2) · Python 41(0)
· 데이터베이스 28(0) · 알고리즘 15(0) · Go 11(0) · 기록 7(5)
```

**`개발 도구` 14편이 `데이터베이스` 28편·`Python` 41편보다 위로 온다.** 사이드바만 훑는 방문자에게 블로그의 무게중심을 잘못 보여준다.
...
| 현재 | 편수 | 이동 |
|---|---:|---|
| `IT` (직속) | 35 | **10곳으로 분해** — 아키텍처/원칙 11 · 보안 5 · 코드 품질/테스트 4 · 개발 도구/에디터·터미널 4 · 인프라/CI·CD 4 · 기록 2 · 나머지 5 |
...
| `IT/AI` | 12 | AI 12 |
| `Kotlin & Java/Spring` | 10 | **인프라/쿠버네티스 6** · Kotlin·Java 4 |
...
| 브라우저 동작 원리 | 네트워크 | 프론트엔드에 가깝지만 이 블로그에 갈 곳이 없다 |
| OOMKilled 추적기 3부작 | 인프라/쿠버네티스 | Spring Boot 앱 문제로 보면 → Kotlin·Java |
| Argo Workflows 전환 3부작 | 인프라/쿠버네티스 | 2편이 Spring Batch REST API 설계라 → Kotlin·Java |
| CDC 4편 | 인프라/데이터 파이프라인 | 전부 "MySQL Debezium"이라 → 데이터베이스/MySQL |
| PHP 2편 | 인프라/리눅스 | 언어 글로 보면 갈 곳이 없다 — 버리거나 별도 상위 |
| ChatGPT + streamlit 웹앱 | Python/라이브러리 | → AI |
| 속도의 병목이 이동하고 있다 | AI | 에세이라 → 기록 |
| 토끼책 2편 | 아키텍처/원칙 | 책 정리라 → 기록 |
| batch 프로그램으로 host 변경 | 개발 도구/에디터·터미널 | hosts 파일이 주제라 → 네트워크 |
| fail2ban | 인프라/리눅스 | 침입 차단이라 → 보안 |

**크롤링이 닿지 않은 것** — 티스토리 관리 화면의 `비공개용 (3)`은 비공개 글이라 실측에 잡히지 않았다. 이 3편은 직접 확인해서 배치하거나 비공개인 채로 남긴다.

**남아 있는 이름 문제** — `알고리즘` 15편 중 13편이 백준·프로그래머스 풀이라 실질은 `코딩테스트`다. 이름을 바꾸면 통계 1편(정규화와 표준화)이 갈 곳을 잃어 그대로 뒀다.
