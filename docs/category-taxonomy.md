...
**상위 13종 · 하위 19종 · 사이드바 32줄** (현재 47줄). 번호가 곧 정렬 순서다 — 규칙은 §4.

| # | 상위 | 편수 | 하위 |
|---:|---|---:|---|
| 1 | **인프라** | 44 | 모니터링 (15) · 데이터 파이프라인 (11) · 리눅스 (8) · 쿠버네티스 (5) · CI·CD (5) |
| 2 | **Kotlin·Java** | 12 | — |
| 3 | **아키텍처** | 24 | 분산 시스템 (13) · 원칙 (11) |
| 4 | **데이터베이스** | 28 | MySQL (19) · NoSQL·검색 (5) · 이론 (4) |
| 5 | **네트워크** | 15 | — |
| 6 | **보안** | 13 | — |
| 7 | **AI** | 19 | — |
| 8 | **코드 품질** | 31 | Clean Code (16) · 리팩토링 (9) · 테스트 (6) |
| 9 | **Python** | 41 | 성능과 동시성 (14) · Django·Flask (10) · 기초 (9) · 라이브러리 (8) |
| 10 | **Go** | 11 | — |
| 11 | **알고리즘** | 15 | — |
| 12 | **개발 도구** | 14 | Git (9) · 에디터·터미널 (5) |
| 13 | **기록** | 8 | — |

### 3.1 축은 둘이다 — 주제, 그리고 언어

**언어를 예외로 두되 일관되게 둔다.** `Python` `Kotlin·Java` `Go` 셋이 같은 레벨에 있고, 나머지는 전부 주제 축이다.
...
**상위는 언어 이름만 쓴다.** `Kotlin·Spring`이 아니라 `Kotlin·Java`인 이유다 — `Python`이 `Python·Django`가 아닌 것과 같다. **프레임워크는 언어 아래로 간다**(`Python/Django·Flask`, `Kotlin·Java` 안의 Spring). 프레임워크를 언어에서 떼어 별도 상위에 두면 "Spring은 웹 프레임워크가 아닌가?" 하는 질문에 답할 수 없다.

### 3.2 배치를 가른 판단들

- **`설계 원칙` 11편은 `아키텍처` 아래로.** SOLID·객체지향·API 설계는 "품질"이 아니라 "설계"다. `아키텍처`가 `분산 시스템`(시스템 설계) + `원칙`(코드 설계)이 되면서 설계가 한곳에 모였고, `코드 품질`은 Clean Code·리팩토링·테스트 31편으로 이름과 내용이 맞는다.
- **`데이터 파이프라인` 11편은 `데이터베이스`가 아니라 `인프라`로.** Kafka·Airflow·Debezium은 저장소가 아니라 운영하는 미들웨어다. **CDC 4편은 원문을 읽어 확인했다** — 90%가 Spring Kafka 클라이언트 구성이고 MySQL 설정은 나오지 않는다.
- **`웹·보안`을 `네트워크`와 `보안`으로 쪼갰다.** 원래 "DNS 작동원리"는 HTTP 쪽에, "통신사 DNS IP 리스트"는 네트워크 쪽에 흩어져 있었다. **`보안`은 최근 2년 4편에 최신 글이 2026.08로 성장 중이다.**
- **OOMKilled 3부작은 `Kotlin·Java`로.** 처음에는 K8s 운영으로 봤지만 **원문을 읽으니 3편이 90% HikariCP 설정이고 해결도 Spring 쪽**이었다. 반대로 **Argo 3부작은 85%가 Argo/K8s YAML**이라 `인프라/쿠버네티스`에 남겼다. 제목이 둘 다 `K8s …`로 시작하지만 내용은 갈렸다.
- **PHP 2편은 `인프라/리눅스`로.** 원문의 90%가 `yum`·`systemctl` 설치 명령이고 Codeigniter 내용은 없다.
- **토끼책 2편은 `아키텍처/원칙`에.** 감상이 아니라 상태·행동·식별자를 정리한 개념 노트다.
- **`AI` 19편은 하위를 두지 않았다.** `LLM 활용`과 `로컬 모델`의 경계가 모호했다 — "Ollama로 Github PR AI 코드리뷰"는 양쪽 다다.
...
| **하위에서 상위 이름을 반복하지 않는다** | `데이터베이스/DB 이론` → `데이터베이스/이론`, `개발 도구/개발 환경` → `개발 도구/에디터·터미널` |
| **상위는 언어 이름만** | `Kotlin·Spring` → `Kotlin·Java`. 프레임워크는 하위에서 다룬다 |
| **한두 단어로 끝낸다** | 사이드바·글 머리 라벨·브레드크럼에 모두 들어간다 |

**`scripts/remap-categories.py`가 앞의 세 규칙을 검사해 위반 시 `exit(1)`** 한다. 축약형(`DB` = `데이터베이스`)은 자동으로 못 잡으므로 스크립트 안 `ALIASES`에 등록해 둔다.

---

## 4. 정렬 순서 — 블로그 주력 순

티스토리 카테고리 관리 화면에서 드래그로 순서를 정한다.

**이건 편집 판단이지 기계적 규칙이 아니다.** 그렇게 부르지 않으면 규칙 이름과 실제 순서가 어긋난 채로 굳는다 — 실제로 한 번 그랬다(아래 각주). 새 카테고리가 생기면 **먼저 몇 군인지 정하고**, 1군이면 손으로 자리를 잡는다.

| 군 | 성격 | 카테고리 |
|---|---|---|
| **1군** | 블로그의 얼굴 — 무엇을 하는 곳인지 보여준다 | 인프라 · Kotlin·Java · 아키텍처 · 데이터베이스 · 네트워크 · 보안 |
| **2군** | 횡단 관심사 | AI · 코드 품질 |
| **3군** | 언어 아카이브 | Python · Go |
| **4군** | 주변 | 알고리즘 · 개발 도구 |
| **5군** | 잡문 | 기록 |

1군 안에서는 **인프라(최대·최활성) → Kotlin·Java(현재 주력) → 아키텍처 → 데이터베이스 → 네트워크 → 보안** 순이다.

### 왜 활성도 순이 아닌가

활성도(최근 2년 글 수)로 정렬하면 `개발 도구` 14편이 `데이터베이스` 28편·`Python` 41편보다 위로 온다. 사이드바만 훑는 방문자에게 블로그의 무게중심을 잘못 보여준다.

활성도 자체는 버리지 않는다 — `python3 scripts/remap-categories.py`가 매번 출력하고, **기본 이미지 도안 우선순위**와 **어느 카테고리를 나중에 쪼갤지** 판단에 쓴다.

> **각주 — 한 번 틀렸던 것** 이 절은 원래 "요청이 들어오는 순서대로 놓는다"라고 적혀 있었는데, 정작 순서는 인프라에서 시작했다. 요청은 네트워크에서 들어온다. 순서를 먼저 정하고 서사를 나중에 갖다 붙이면 이렇게 된다.
...
| 현재 | 편수 | 이동 |
|---|---:|---|
| `IT` (직속) | 35 | **10곳으로 분해** — 아키텍처/원칙 11 · 보안 5 · 인프라/CI·CD 4 · 개발 도구/에디터·터미널 4 · 코드 품질/테스트 3 · 기록 2 · 나머지 6 |
| `Python/Python` | 28 | Python/성능과 동시성 14 · Python/기초 9 · Python/라이브러리 4 · 알고리즘 1 |
| `Database/MySQL` | 21 | 데이터베이스/MySQL 17 · 데이터베이스/이론 4 |
| `IT/Web` | 18 | 네트워크 8 · 보안 8 · 아키텍처/분산 시스템 1 · 기록 1 |
| `IT/알고리즘` | 14 | 알고리즘 13 · 아키텍처/분산 시스템 1 |
| `IT/AI` | 12 | AI 11 · 기록 1 |
| `Kotlin & Java/Spring` | 10 | **Kotlin·Java 7** · 인프라/쿠버네티스 3 |
| `OS/Centos` | 10 | 인프라/리눅스 6 · 인프라/모니터링 2 · 네트워크 2 |
| `IT/Git` | 9 | 개발 도구/Git 8 · 인프라/CI·CD 1 |

**통째로 옮겨가는 것** — `IT/Clean Code`→코드 품질/Clean Code, `IT/리팩토링`→코드 품질/리팩토링, `IT/Langchain`→AI, `Infrastructure/MSA`→아키텍처/분산 시스템, `Infrastructure/Fluentd`→인프라/모니터링, `Go`→Go, `Kotlin & Java`(직속)→Kotlin·Java, `Python/Django`+`Python/Flask`→Python/Django·Flask, `PHP/*`→인프라/리눅스, `Infrastructure/Kafka`+`CDC`+`Airflow`→인프라/데이터 파이프라인.
...
제목만으로는 갈렸던 11건을 **원문을 읽어 확정했다**(2026-08-25). **4건이 처음 배치와 달랐다.**

| 글 | 처음 배치 | 확정 | 원문 확인 결과 |
|---|---|---|---|
| OOMKilled 3부작 | 인프라/쿠버네티스 | **Kotlin·Java** | 3편이 90% HikariCP 설정이고 해결도 Spring 쪽 |
| 캐시 (Cache) | 네트워크 | **아키텍처/분산 시스템** | HTTP 캐시도 Redis도 아닌 일반 컴퓨팅 개념(RAM·스토리지 계층) |
| 속도의 병목이 이동하고 있다 | AI | **기록** | 코드 없는 1인칭 에세이 |
| 브라우저 동작 원리 | 네트워크 | 네트워크 (미해결) | 렌더링 엔진 내부가 중심. 아래 참조 |
| Argo Workflows 3부작 | 인프라/쿠버네티스 | 그대로 | 85%가 Argo/K8s YAML, Spring 코드 없음 |
| CDC 4편 | 인프라/데이터 파이프라인 | 그대로 | 90%가 Spring Kafka 클라이언트, MySQL 설정 없음 |
| ChatGPT + streamlit | Python/라이브러리 | 그대로 | streamlit UI 사용법 중심, ChatGPT는 토큰 입력만 |
| 토끼책 2편 | 아키텍처/원칙 | 그대로 | 감상이 아니라 상태·행동·식별자 개념 노트 |
| batch로 host 변경 | 개발 도구/에디터·터미널 | 그대로 | 윈도우 `.bat`으로 로컬 환경 전환 |
| fail2ban | 인프라/리눅스 | 그대로 | 설치·설정 가이드, 보안 개념 없음 |
| PHP 2편 | 인프라/리눅스 | 그대로 | 90%가 `yum`·`systemctl`, Codeigniter 내용 없음 |

**OOMKilled와 Argo가 갈린 것이 이번 확인의 소득이다.** 제목이 둘 다 `K8s …`로 시작해 같이 묶었는데, 원문은 한쪽이 Spring 설정 문제, 다른 쪽이 K8s 운영 문제였다.

### 아직 갈 곳이 마땅치 않은 1건

**브라우저 동작 원리** — 원문은 렌더링 엔진 내부(파싱·DOM·렌더트리)가 중심이고 네트워크는 항목 하나다. 이 블로그에 프론트엔드 카테고리가 없어 갈 곳이 없다. 1편을 위해 카테고리를 만들 수는 없어 `네트워크`에 둔다.

### 남아 있는 이름 문제

`알고리즘` 15편 중 13편이 백준·프로그래머스 풀이라 실질은 `코딩테스트`다. 이름을 바꾸면 통계 1편(정규화와 표준화)이 갈 곳을 잃어 그대로 뒀다.

### 크롤링이 닿지 않은 것

티스토리 관리 화면의 `비공개용 (3)`은 비공개 글이라 실측에 잡히지 않았다. 이 3편은 직접 확인해서 배치하거나 비공개인 채로 남긴다.
...
| **DESIGN.md §6.2** (`data-cat` 접두 선택자) | **선택자 값 전면 교체.** `&`가 사라져 이스케이프 걱정이 줄었지만 공백이 있으므로 여전히 따옴표로 감싼다 — `[data-cat^="개발 도구"]`. 새 상위 13종 사이에 접두 충돌은 없다 |
