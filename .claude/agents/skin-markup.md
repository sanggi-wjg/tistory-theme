...
- **영역 치환자를 올바른 위치에 놓는다.** 목록은 홈까지 `<s_list>`/`<s_list_rep>` 한 벌로 그린다 — `<s_list>`는 홈에서도 렌더된다(결정 29). `<s_permalink_article_rep>`·`<s_index_article_rep>`는 독립 영역이 아니라 **`<s_article_rep>`의 하위 영역**이다. 바깥에 두면 티스토리가 에러 없이 통째로 버린다 — 2026-08-25 첫 배포에서 글 본문이 사라진 원인이었다(린트 `SUB008`). `*_group` 치환자는 자기 래퍼(`<s_rp>`·`<s_guest>`) 안에 둔다(결정 34, `SUB009`).
...
- **티스토리가 통짜로 렌더링하는 영역은 건드리지 않는다.** 카테고리(`[##_category_list_##]` — 폴더형 `[##_category_##]`이 **아니다**, 결정 31·린트 `CAT001`), 댓글(`[##_comment_group_##]`), 방명록(`[##_guestbook_group_##]`)은 치환자 한 줄만 놓고 스타일은 skin-style에 맡긴다.
...
`data-cat`이 없으면 카테고리별 기본이미지가 전부 무너진다. 홈도 같은 `<s_list_rep>` 마크업이므로 값이 같다(결정 29).
