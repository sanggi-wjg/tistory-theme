---
name: skin-preview
description: "티스토리 스킨을 로컬 브라우저에서 확인하는 mock 렌더러. src/skin.html의 치환자를 data/posts.json의 실제 전수 글 데이터로 치환해 홈·글(목차 유/무 2종)·카테고리·검색·태그(목록/클라우드 2종)·보관함·방명록·검색결과0건·보호글·부속이 빈 글 12개 페이지를 생성한다. 스킨을 수정한 뒤 '어떻게 보이는지 보자', '프리뷰', '미리보기', '렌더링해봐', '화면 확인', '브라우저로 열어봐' 요청 시 반드시 이 스킬을 사용할 것. 티스토리 API를 쓰지 않으므로 이것이 유일한 확인 수단이다."
---

# 로컬 프리뷰 — 치환자 mock 렌더러

브라우저는 `<s_list_rep>`를 모른다. `skin.html`을 그냥 열면 깨진 화면이 나온다. 이 스킬은 티스토리 서버가 하는 치환을 로컬에서 흉내내, **실제 글 전수 데이터**(`data/posts.json`)로 렌더링한다.
...
| `page_bare` | `tt-body-page` | **부속이 빈 글.** 태그 0 · 관련글 0 · **다음 글 없음(이전 글은 있다)** · 공지 0 · `[##_blog_menu_##]`가 **빈 `<ul></ul>`**(=**라이브의 실제 조건** — 빈 문자열이 아니다. `:empty`가 아니라 `:has(> ul:empty)`가 잡아야 한다, 결정 57). 값이 비면 장식만 남는 실패(결정 42)가 태어난 부류인데 2026-08-28까지 이 조건을 그리는 화면이 없었다. 전부 실블로그에서 나오는 상태다 — 태그를 안 단 글, 그 카테고리에 글이 하나뿐인 글, 첫 글·마지막 글. **이전·다음을 둘 다 지우지 않는 이유**는 그런 글이 없기 때문이다(글이 한 편인 블로그에서만 나온다) — 실재하지 않는 화면을 보고 판단하면 그 판단이 헛것이 된다 |
...
static/pc/dist/index.css      ← 우리보다 뒤 (아래 단서를 읽어라)
```

**이것이 없으면 프리뷰는 거짓말을 한다.** 다크에서 인용문이 사라지는 종류의 결함은 원인이
우리 CSS가 아니라 티스토리 CSS와의 특이도 싸움이라, 상대를 안 부르면 멀쩡해 보인다.
이 저장소는 프리뷰가 통과 신호를 위조한 사고를 여러 번 겪었다(횟수·목록은 CLAUDE.md 「핵심 위험」이 정본).

순서도 검사의 일부다. `atom-one-light`이 우리 뒤에 와야 `.hljs ` 접두가 정말 필요한지 눈으로 확인된다.
**여기 순서를 바꾸지 말 것.**

`static/pc/dist/index.css`는 **모든 페이지** head에 링크된다(2026-09-10 재실측: 홈·방명록·글 셋 다).
글 페이지 전용인 것은 시트가 아니라 **Namecard div**다 — 둘을 섞으면 `V017`이 엉뚱한 페이지에서
시트를 찾는다. 이 시트는 티스토리 **React 앱**들의 것이다 — 댓글(`Comment`)과 프로필
카드(`Namecard`)가 한 파일에서 온다. 2026-09-10까지 이 시트가 빠져 있어, 소스에 빈 껍데기뿐인
두 앱의 특이도 싸움이 **로컬에서 한 번도 벌어지지 않았다.** 이슈 #59(다크에서 프로필 박스가
밝은 회색 판)가 그 침묵에서 나왔다.

⚠ **라이브 `<head>`에서는 이 시트가 우리 앞인데, 프리뷰는 일부러 뒤에 싣는다.**
`tistory.css` 댓글 블록의 2026-08-26 실측은 정반대("티스토리 시트가 우리 뒤")였다 —
순서는 티스토리가 정하고 예고 없이 바뀐다. 뒤에 싣는 쪽이 더 엄격한 조건이라(같은 특이도면
뒤가 이긴다) 여기서 이기면 어느 순서에서도 이긴다. 그래서 **프리뷰에서 멀쩡한 것으로는
부족하고** 린트 `TIS003`·`TIS005`가 특이도 자체를 따로 본다.
이 시트는 클래스 스코프 규칙뿐이라(bare 요소 선택자 0개) 다른 화면을 흔들지 않는다.

네트워크가 없으면 세 시트가 조용히 빠진다. 그때는 화면 하단에 **주황색 경고 띠**가 뜬다 —
띠가 보이면 지금 보고 있는 것은 반쪽이고, 특이도 관련 판단을 내리면 안 된다.

## 페이징은 세 모양을 그린다

`[##_paging_rep_link_num_##]`은 **숫자가 아니라 `<span class="selected">N</span>`**으로
치환된다(2026-09-10 실측). 현재 페이지 표시는 그 `span` 하나뿐이고, 생략 부호 `···`도
`a.paging-num`으로 오되 **href가 없다**. 더 갈 곳이 없을 때 붙는 클래스는
**`no-more-prev`/`no-more-next`(하이픈)** 이며 그 앵커에도 href가 없다.

렌더러가 2026-09-10까지 1~5를 맨 숫자로 냈다 — 그래서 **현재 페이지 강조가 없다는 것(이슈 #58)을
로컬에서 볼 방법이 없었고**, 우리 CSS가 밑줄 이름(`.no_more_prev`)을 보고 있는 것도 가려졌다.

| 페이지 | 현재 | 그리는 조건 |
|---|---|---|
| `index` | 1 / 22 | `1(selected) 2 3 4 ··· 22` · **이전 비활성**(`no-more-prev`, href 없음) |
| `category`·`search`·`tag` | 9 / 22 | `1 ··· 6 7 8 9(selected) 10 11 12 ··· 22` · 양쪽 활성 (라이브 `/?page=9` 그대로) |
| `archive` | 22 / 22 | `1 ··· 19 20 21 22(selected)` · **다음 비활성**(`no-more-next`) |

세 모양 모두 라이브 실측이다 — `/`(1페이지)·`/?page=9`·`/?page=22`(마지막). 마지막 페이지는
`<a class="paging-next no-more-next" >다음</a>`(href 없음)과 `1 ··· 19 20 21 22`를 내놓았고,
`paging_items(22)`가 같은 목록을 만든다.
...
| **방명록·댓글 0건 화면** | 위와 같은 이유다. 「댓글이 없습니다」류 문구도 React가 낸다 — `page_bare`가 그리는 것은 **우리 마크업의** 빈 상태(태그·관련글·이전다음·메뉴)이지 티스토리 앱의 빈 상태가 아니다 |
...
렌더러를 고친 뒤에는 **12개 페이지를 모두 다시 생성**해 경고가 늘지 않았는지 확인한다.
...
- **페이지를 더할 때는 «실재하는 상태»만 그린다.** `page_bare`의 첫 판은 이전·다음을 둘 다 지웠고,
  그러자 `nav.postnav`가 자식 없이 `margin-top: 32px`만 차지하는 화면이 나왔다. 고칠 뻔했지만
  **그런 글이 없다** — 첫 글도 마지막 글도 한쪽은 있고, 둘 다 없으려면 글이 한 편이어야 한다.
  있지도 않은 조건을 보고 규칙을 만들면 검사만 늘고 막는 것은 없다. 지금은 한쪽(다음 글)만 지워
  첫 글·마지막 글에서 **매번 나오는** 상태를 그린다 (`.postnav`는 한 칸짜리 그리드라 자식이
  하나여도 정상이다 — 확인함).
