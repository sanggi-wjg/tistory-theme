---
name: skin-preview
description: "티스토리 스킨을 로컬 브라우저에서 확인하는 mock 렌더러. src/skin.html의 치환자를 data/posts.json의 실제 275편 데이터로 치환해 홈·글(목차 유/무 2종)·카테고리·검색·태그(목록/클라우드 2종)·보관함·방명록·검색결과0건·보호글·부속이 빈 글 12개 페이지를 생성한다. 스킨을 수정한 뒤 '어떻게 보이는지 보자', '프리뷰', '미리보기', '렌더링해봐', '화면 확인', '브라우저로 열어봐' 요청 시 반드시 이 스킬을 사용할 것. 티스토리 API를 쓰지 않으므로 이것이 유일한 확인 수단이다."
---
