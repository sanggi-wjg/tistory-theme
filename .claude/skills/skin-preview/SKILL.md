...
| `page_bare` | `tt-body-page` | **부속이 전부 빈 글.** 태그 0 · 관련글 0 · 이전다음 없음 · 공지 0 · `[##_blog_menu_##]` 빈 값(=**라이브의 실제 조건**). 값이 비면 장식만 남는 실패(결정 42)가 태어난 부류인데 2026-08-28까지 이 조건을 그리는 화면이 없었다. 태그를 안 단 글·그 카테고리에 글이 하나뿐인 글·첫 글/마지막 글에서 실제로 나온다 |
...
- **`page_bare`에서 관찰된 것(2026-08-28)** — 부속이 전부 없으면 `nav.postnav`가 자식 없이 남아
  `margin-top: 32px`만 차지한다. `:empty`로는 못 잡는다(치환자가 제 줄에 있어 공백 텍스트 노드가
  남는다 — `layout.css`가 경고하는 그 함정이다). **가드를 넣지 않았다**: 이 상태가 되려면 이전·다음이
  **둘 다** 없어야 하는데 글 276편에서 그런 글은 없다(첫 글·마지막 글도 한쪽은 있다). 글이 한 편뿐인
  블로그에서만 나온다. 조건이 실재하게 되면 `:has()` 가드를 넣는다 — 있지도 않은 조건에 규칙을 만들지 않는다
