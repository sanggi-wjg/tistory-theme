...
| `page_bare` | `tt-body-page` | **부속이 빈 글.** 태그 0 · 관련글 0 · **다음 글 없음(이전 글은 있다)** · 공지 0 · `[##_blog_menu_##]` 빈 값(=**라이브의 실제 조건**). 값이 비면 장식만 남는 실패(결정 42)가 태어난 부류인데 2026-08-28까지 이 조건을 그리는 화면이 없었다. 전부 실블로그에서 나오는 상태다 — 태그를 안 단 글, 그 카테고리에 글이 하나뿐인 글, 첫 글·마지막 글. **이전·다음을 둘 다 지우지 않는 이유**는 그런 글이 없기 때문이다(글이 한 편인 블로그에서만 나온다) — 실재하지 않는 화면을 보고 판단하면 그 판단이 헛것이 된다 |
