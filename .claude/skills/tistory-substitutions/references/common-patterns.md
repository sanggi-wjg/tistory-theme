...
`[##_article_related_rep_type_##]`은 `text_type` / `thumb_type`을 출력한다. 티스토리가 썸네일 유무를 알려주는 것이지만, **이 스킨은 그걸로 칸을 켜고 끄지 않는다**(결정 57) — 썸네일 **상자는 그룹 치환자 밖**에, `<img>`만 안에 둔다(린트 `BND011`). 위 예제는 치환자 위치만 보여 주는 것이라 상자를 그리지 않았다. 상자를 그룹 안에 넣으면 대표이미지 없는 줄이 왼쪽으로 튄다.
...
**출력은 이름과 다르다**(2026-09-10 라이브 실측, 결정 53): ① `[##_paging_rep_link_num_##]`은 맨 숫자가 아니라 `<span class="selected">N</span>`(현재 페이지) / `<span class="">N</span>`이다 — 현재 위치 신호는 이 `span.selected`뿐이다. ② 생략 부호 `···`도 `<a>`로 나오는데 **href가 없다**. ③ `[##_no_more_prev_##]`가 내는 클래스는 **`no-more-prev`(하이픈)** 이지 밑줄이 아니다. 밑줄 이름을 CSS에 쓰면 라이브에서 한 번도 매칭되지 않는다.
