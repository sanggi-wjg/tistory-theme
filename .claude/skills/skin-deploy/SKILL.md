...
python3 .claude/skills/skin-preview/scripts/render.py   # 12개 페이지 육안 확인
...
| 1-c | `dist/images/ph-<slug>-<theme>.v<N>.webp` **30장** | 스킨 편집 → **파일업로드** | **기본 이미지가 바뀐 배포에서만** (결정 5·6 개정). 반드시 **style.css보다 먼저** — 반대면 그 사이 카드가 옛 도안(폴백)으로 뜬다. **폴백 때문에 빠진 장이 화면에서 티 나지 않는다** — 올린 뒤 `curl -I`로 30장이 200인지 반드시 본다. 버전(N)이 올랐으면 새 이름으로 올리고 옛 버전 30장은 지운다. 2026-08-27 첫 배포에서 31 + 4개를 거부 없이 받았다(닫힌 미결 1) |
...
- [x] ~~**페이징 현재 페이지 클래스를 모른다**~~ — 2026-09-10 확정(결정 53). `[##_paging_rep_link_num_##]`은
      숫자가 아니라 `<span class="selected">N</span>`으로 치환된다 — **현재 페이지 표시는 이 span뿐이고**
      앵커에는 아무 표시도 안 붙는다. 생략 부호 `···`와 끝 페이지의 비활성 화살표는 **href가 없다.**
      갈 곳 없을 때 붙는 클래스는 `no-more-prev` / `no-more-next`(**하이픈**) — 밑줄 이름은
      라이브에 존재하지 않는다. `components.css`에 대응 규칙이 있고 프리뷰가 세 모양을 그린다
- [ ] **페이징 세 모양** — 배포 후 홈 **1 · 9 · 22페이지**를 직접 열어 본다. 1페이지에서 "이전"이
      흐린가(opacity .35), 9페이지에서 현재 번호에 알약 배경 + `--link` 글자가 있는가, 22페이지에서
      "다음"이 흐린가, 생략 부호에 테두리가 없고 클릭되지 않는가
...
- [ ] **홈 카드에 삽화가 스치지 않는가**(결정 57 ②) — 라이브 홈을 **두 번 새로고침**해 대표이미지 있는 카드에서
      카테고리 삽화가 보였다가 사진으로 바뀌지 않는지 본다. `loading="lazy"` 타이밍이라 **정적 프리뷰로는
      원리적으로 못 본다**. 라이브 확인이 유일한 수단이다
- [ ] **관련 글·사이드바·이전다음에서 대표이미지 없는 줄이 왼쪽으로 튀지 않는가**(결정 57 ①) — 상자가
      그룹 밖에 있어야 한다. 린트 `BND011`이 소스는 막지만 붙여넣기가 잘리면 소스와 다르다
- [ ] **헤더 높이** — 메뉴를 설정하지 않은 상태에서 헤더 위에 빈 줄이 없는가(결정 57 ③, `:has(> ul:empty)`)
- [ ] **카테고리 새 글 표시**(결정 58) — 새 글이 있는 가지에 파란 점이 이름 바로 뒤에 붙고, 흰 상자나
      GIF가 아니며, 글 수 배지가 모든 줄에서 오른쪽 끝에 정렬되는가. 다크에서도 본다. 하위 카테고리에
      새 글이 있을 때의 모양은 **미측**이다 — 보이면 적는다
- [ ] **인쇄·고대비**(결정 52) — 인쇄 미리보기에서 종이가 라이트 팔레트인가. 고대비(강제 색상)는 OS 설정이
      필요해 확인 못 했으면 **미검증**으로 적는다
...
| **부분 확인** | 스킨 편집기의 파일 개수·용량 제한은 공식 문서에 없다. 35개·약 0.5MB까지는 받는 것을 2026-08-27에 확인했다(`DECISIONS.md` 닫힌 미결 1). 그 위의 상한은 모른다 |
