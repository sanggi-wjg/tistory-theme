...
```bash
# 목록만 (빠르다 — 빈 페이지가 나올 때까지, 2026-09-15 라이브 22쪽)
python3 .claude/skills/blog-census/scripts/census.py --posts

# 본문까지 (느리다, 전수 — 인라인 스타일·코드블록 집계에 필요)
...
| 함정 | 옳은 방법 |
|---|---|
| RSS로 전수를 세려 함 | RSS는 **최신 50편만** 준다. 목록 페이지를 빈 쪽이 나올 때까지 훑어야 전수가 나온다(구 스킨 20편/쪽 14장 → 우리 스킨 13편/쪽 22장) |
