...
### 린트 자신을 검사하는 것 — `npm run check`의 `test:*` 단계들
...
(14개 케이스). `BND008`은 같은 방식에 **오탐 케이스를 더한다**(9개 중 3개) — 첫 판이
...
이 저장소는 그 차이로 여러 번 데었다(횟수와 목록은 CLAUDE.md 「핵심 위험」이 정본). §5.6 표나 예외 목록의 모양을
...
| `BND001~005` | `data-cat` 경계면, 카테고리 커버리지, JS 셀렉터 ↔ 마크업(`004` — **class 속성의 토큰과 정확 일치.** 2026-08-27까지 접두·산문 일치라 `entry-body`를 `entry-body-v2`로 개명해도 통과했다. §5.6·§8에 등재된 JS 생성 클래스는 제외), **`[class="contents_style"]` 정확일치**(`005`) |
...
| `DOC001` | **문서(`*.md` 전부)의 `파일:줄번호` 인용 (오류).** 줄번호는 그 위에 한 줄만 들어가도 조용히 다른 줄을 가리킨다 — 2026-08-26 한 세션에서 5건이 썩어 있었고 그중 둘은 **쓴 PR에서 리뷰가 놓쳤다**, 2026-08-27 전체 대조에서 20건 중 1건. `` `code.js`의 `LANGS` `` 같은 심볼 인용은 줄이 밀려도 안 썩는다. **심볼의 존재는 검사하지 않는다** — 이름을 바꾸면 저장소 전체를 훑는 규범(CLAUDE.md)이 이미 있고, 검증기 비용이 남은 인용 수를 넘는다. 스킨이 아니라 문서를 보는 유일한 규칙이라 `skin.html`이 없어도 돈다. `test-syntax-checks.py`에 켜지는 케이스 2·오탐 케이스 1 (결정 54) |
...
### 페이지 커버리지
12개 페이지가 **빈 화면 없이** 나오는가. 특히 `empty`(검색 결과 0건)·`guestbook`·`protected`는 빠뜨리기 쉽다.
...
### 반응형
640 / 768 / 1024 / **1280** / 1440px. 1280은 레일은 서고 목차 칸은 없는 구간이라 따로 본다(결정 48). **페이지 본문이 가로로 스크롤되면 실패다.** 코드블록·표는 자기 컨테이너 안에서만 스크롤해야 한다. 홈 카드 제목이 2줄에서 잘리는지도 본다(홈 노출 제목 중앙값 약 50자).
...
지적은 **파일과 심볼**(선택자·치환자·함수 이름)과 수정 방법을 포함한다. **줄번호는 쓰지 않는다** — 위에 한 줄만 들어가도 다른 줄을 가리키는데 화면에 신호가 없다(결정 54, 린트 `DOC001`).

```
❌ src/skin.html 의 .post-list 항목이 class="card"를 출력하는데
   src/styles/components.css 는 .post 를 찾는다.
