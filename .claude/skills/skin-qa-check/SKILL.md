...
### 반응형
640 / 768 / 1024 / **1280** / 1440px. 1280은 레일은 서고 목차 칸은 없는 구간이라 따로 본다(결정 48). **페이지 본문이 가로로 스크롤되면 실패다.** 코드블록·표는 자기 컨테이너 안에서만 스크롤해야 한다. 홈 카드 제목이 2줄에서 잘리는지도 본다(홈 노출 제목 중앙값 49자).
