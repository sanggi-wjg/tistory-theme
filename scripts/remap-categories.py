...
**백엔드 스택 계층 순**으로 놓았다 — 규칙은 docs/category-taxonomy.md §4.
...
이름 규칙 — `&`를 쓰지 않는다(URL에서 %26, RSS·OG에서 이스케이프).
...
TAXONOMY = [
    ...
    ("아키텍처", [
        (None,                [31,32,33,95,151,152,153,154,155,156,157,158]),
    ]),
    ("데이터베이스", [
...
# ── 활성도 (정렬 규칙의 근거는 아니지만 기본 이미지·문서 갱신에 쓴다) ──
print("\n=== 카테고리 활성도 ===")
...
json.dump(
    {"note": "TAXONOMY 순서 = 티스토리 카테고리 정렬 순서 (백엔드 스택 계층 순)",
