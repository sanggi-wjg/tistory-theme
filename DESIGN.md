...
⚠ **글 페이지 전용인 것은 `div`뿐이고 시트는 아니다.** `index.css`는 홈·방명록 head에도 링크된다(2026-09-10 실측). 「Namecard가 글 페이지에만 있으니 그 규칙도 글 페이지에만 있다」로 읽으면 안 된다 — 이 시트에는 댓글 앱 규칙도 함께 들어 있고, 그쪽은 방명록에서 실제로 쓰인다.
...
- `.tt_ico_cross`(구독 전의 `+`)와 `.tt_ico_check`(**구독 중** `.type2`의 체크)는 스프라이트 이미지 아이콘이라 색 토큰이 못 닿는다. 짙은 회색이라 다크에서 안 보였다(2026-09-10 라이브 주입 실측) — `--sprite-invert`(§2 "색이 아닌 값")로 **둘 다** 반전시킨다. 한쪽만 하면 로그인한 구독자에게만 반쪽으로 깨진다. `.tt_ico_check`는 로그인 상태에서만 나와 **미검증**이다.
...
- **언어 라벨은 우상단에 표시**한다. 어느 값을 쓰는지는 **누가 썼는가**로 갈린다 (결정 43):
  ...
  - **에디터가 붙인 것은 무엇이든 무시한다** — `data-ke-language`(286개/39%, `javascript` 44개가 전부 오답)와 `<pre>`의 클래스(전수 527개/72%에 이 블로그에 없는 언어 46개 — 결정 55) 둘 다.
