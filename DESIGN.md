...
:root {
  ...
  --ink:           #171717;
  --ink-body:      #4d4d4d;
  --ink-mute:      #767676;   /* 2026-08-25 #888888에서 내림 — §8.1 참조 */
  --link:          #0070f3;
  --link-deep:     #0761d1;
  --link-bg-soft:  #d3e5ff;
  --error:         #ee0000;
...
| 2026-08-25 | `--ink-mute` 라이트 AA 미달 | `#888888` → **`#767676`** (흰 배경 4.54:1). 다크 `#8f8f8f`는 6.49:1로 통과라 그대로 뒀다. 12px 캡션 20여 곳이 걸려 있었다. **`--ink-mute`를 크롬 라벨에만 쓴다는 §2 사용 규칙은 그대로 유효하다** — 4.54:1은 AA 하한을 겨우 넘긴 값이라, 읽어야 하는 텍스트는 계속 `--ink-body`를 쓴다 |
