...
**인쇄에는 스크롤이 없다** — `@media print`에서 코드블록·표를 `pre-wrap`으로 풀고 크롬(레일·목차·진행바·맨위로·페이징·댓글·복사 버튼·광고·푸터)을 지운다. 팔레트는 `tokens.css`가 라이트로 되돌린다(결정 52).
...
| 날짜 | 항목 | 무엇을 했나 |
|---|---|---|
| 2026-08-28 | 인쇄 스타일이 **한 줄도 없었다** · 강제 색상 모드에서 진행바가 사라진다 · 복사 버튼이 24×24 하한에 1px 모자람 | `@media print`가 토큰을 라이트로 되돌리고(세 선택자 — 다크가 두 블록에서 온다) 크롬을 지운다. 레일은 상대가 ID 스코프라 **켜는 쪽**을 `@media not print`로 좁혔다(`!important`는 인라인 보정 전용). `.reading-progress-bar`에 `forced-color-adjust: none` + `Highlight`. `.code-copy` 세로 패딩 3 → 5px. `.heading-anchor`는 인라인 예외라 그대로. 결정 52 |
