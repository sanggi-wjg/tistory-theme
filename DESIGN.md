...
페이지를 옮기면 자리가 바뀌었다. 사이드바를 통째로 왼쪽에 세워 **전 페이지에서
카테고리가 같은 x좌표에 온다** (실측 1440px: 레일 x=44, 폭 240 — 홈·글·목록 동일).

그래서 `--page-w`도 레일이 서는 페이지에서는 전부 `--wrap`(1400px)이다. 페이지마다
...
| 1025~1239px | **좌측** | 2열 (카드 299px) | 목차 접이식 |
| 1240~1399px | 좌측 | **3열** | 목차 접이식 |
| 1400px~ | 좌측 | 3열 (카드 339px) | **우측 목차 (3단)** |

두 하한의 근거는 실측이다.

- **홈 3열 1240px** — `(1240 - 여백 48 - 레일 240 - 간격 48 - 카드간격 48) / 3 = 285px`.
  280px 아래로 가면 카드 제목이 두 줄에서 매번 잘린다. 1025~1239px에서는 2열이고
  카드가 오히려 넓다.
- **글 3단 1400px** — `240 + 32 + 800 + 32 + 240 + 여백 48 = 1392`. 레일 폭이 300px이면
  1460px이 필요해 1440 모니터가 3단에서 떨어진다. 그래서 `--sidebar-w`를 240px로 정했다.
...
- **구문 하이라이팅 색 규정이 없다.** §6.3은 "언어 라벨"과 "신뢰도"만 정하고 `.hljs-*` 토큰 색은 정하지 않는다. 그래서 `components.css`는 기존 토큰만으로 2톤(잉크 사다리 + `--link`) 팔레트를 임시로 짜 두었다. 코드블록 728개가 사실상 무채색으로 보인다는 뜻이다. 제대로 하려면 **코드 전용 색 토큰 4~5종**(문자열·숫자·주석·키워드)을 라이트/다크 양쪽에서 대비를 재고 추가해야 한다.
  **범용 토큰을 빌려 쓰는 대가는 2026-08-26에 한 번 청구됐다** — `--link`·`--error`가 라이트에서 AA 미달이었고, 둘 다 코드블록 배경 위에서만 드러났다(§8.1). 지금은 통과하지만, **다음에 누가 `--link`를 "링크니까 흰 배경 기준으로" 다시 조정하면 코드블록이 조용히 같이 떨어진다.** 코드 전용 토큰을 만들면 그 연결이 끊긴다 — 그것이 이 미결의 진짜 값어치다.
