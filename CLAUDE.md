...
| 2026-08-25 | 스킨 미리보기 이미지 4종 + `dist/skin.zip` 패키징 | src/preview/, scripts/gen-preview.mjs, scripts/build.mjs, skin-deploy, DECISIONS.md(미결 7) | 관리 화면과 스킨 보관함에 엑박이 떴다. preview*는 스킨 **루트** 파일인데 편집기의 파일업로드 탭은 `images/` 아래에만 올린다 — 편집기로는 넣을 수 없어 보관함 직접 업로드(zip)가 유일한 경로다 |
