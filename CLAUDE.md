...
**검증 도구가 통과 신호를 위조한 적이 세 번 있다.** 프리뷰가 영역 중첩 규칙을 몰랐고, 카테고리 치환자 두 종류를 같은 마크업으로 그렸고, 티스토리 시트를 실지 않아 특이도 싸움이 재현되지 않았다. 셋 다 실제 배포에서만 드러났다. **린트·프리뷰가 통과하면 "그 검사가 실제 조건을 재현하는가"부터 의심하고, 재현하지 않으면 검사부터 고친다.**
