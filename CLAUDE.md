...
| 2026-08-27 | `npm run check`가 빌드를 두 번 돌리던 것 제거(`preview:only`), CI 아티팩트에 `dist/` 동봉(프리뷰가 `../../dist/`를 참조), node를 `.nvmrc`(26)·`engines`(>=20)로 핀, 낡은 횟수·수치 정정(「세 번」·「14케이스」·「10페이지」·hooks.md §9-2) | `package.json`, `.nvmrc`, `.github/workflows/check.yml`, README·USAGE, 스킬 문서 셋, `docs/hooks.md` | 같은 리뷰. 위성 문서가 CLAUDE.md의 횟수를 복사해 두어 갈라졌다 — 이제 횟수는 인용하지 않고 「CLAUDE.md 「핵심 위험」」을 가리킨다 |
